...
final class eliminadosControlador extends Controlador
{
    ...
    public function __construct() {
        $this->construir();
        $this->_modelo = $this->cargarModelo('eliminados');
	...
	public function index(int $pagina = 0): void
	{
		try {
            ...
            $this->_vista->asignar('titulo', 'Usuarios Eliminados');
        ...
        $this->_vista->setJS(['tabla', 'botones', 'usuarios']);
        $this->_vista->setCSS(['usuarios', 'perfiles']);
		$this->cargarPagina($this->_vista->renderizar('eliminados', 'usuarios', $this->_plantilla));
	}
	...
	private function usuarios(int $pagina, int $limite = 25, string $orden = 'ASC', string $campo = 'u.id', string $search = '')
	{
		...
		$tabla = $this->cargarLibreria('TablaBlockpc', 'Usuarios Eliminados', 8);
		...
		$tabla->setVistaTabla($this->_rutaVista . 'usuario.phtml');
    ...
    public function activar()
    {
        try {
            ...
            $fecha = date_create('U');
            $updated_at = date_format($fecha, 'Y-m-d H:i:s');
            $this->_modelo->activar($id, $updated_at, $user_id);
            $resultado['ok'] = true;
        } catch(\Exception $e) {
            $resultado['ok'] = false;
            $resultado['error'] = $e->getMessage();
